...
No resistors needed. The Arduino <-> ESP32 link is ONE-WAY only
(ESP32 sends motor commands, Arduino never talks back), so there's
no 5V-into-3.3V-pin risk and no voltage divider required.
...
[ ] ESP32 GPIO17 (TX) -> Arduino pin 2 (one wire, no resistor needed)
[ ] Arduino GND <-> ESP32 GND, shared ground rail on the breadboard
    (this one is easy to forget and will make the link fail silently)
...
Note: since the Arduino never sends anything back to the ESP32, there's
no way for the ESP32 to know if the Arduino actually received a command.
If a rover ever seems "stuck" mid-trial, first check the one TX wire and
the shared ground connection before assuming it's an algorithm problem.
