...
const App = () => {
  ...
  const [momentum, setMomentum] = useState(0.88);
  const [scanRadius, setScanRadius] = useState(5);
  const [targetSamples, setTargetSamples] = useState(2000);
  ...
  const runSimulationStep = useCallback((population, repulsion, mom, radius) => {
    ...
    const loop = () => {
      if (isRunning && stats.current.count < targetSamples) {
        ...
        while (performance.now() - start < CONFIG.FRAME_BUDGET_MS) {
          ...
          const result = runSimulationStep(agentCount, repulsionStrength, momentum, scanRadius);
  ...
  }, [isRunning, agentCount, repulsionStrength, momentum, scanRadius, targetSamples, runSimulationStep]);
  ...
  const exportCSV = () => {
    ...
    const header = "trial,efficiency,steps,coverage_pct,agents,repulsion_g,momentum,scan_radius\n";
    const rows = s.allTrials.map(t =>
      `${t.trial},${t.efficiency.toFixed(6)},${t.steps},${(t.coverage * 100).toFixed(2)},${t.agents},${t.repulsion_g},${t.momentum},${t.scan_radius}`
    ).join("\n");
    // filename encodes all key params
    const muStr = String(momentum).replace('.','_');
    const gStr  = String(repulsionStrength).replace('.','_');
    const fname = `vel_v4_3_N${agentCount}_MU${muStr}_R${scanRadius}_G${gStr}_${s.count}trials.csv`;
    triggerDownload(header + rows, fname, 'text/csv');
  };
  ...
  const exportJSON = () => {
    ...
    const muStr = String(momentum).replace('.','_');
    const gStr  = String(repulsionStrength).replace('.','_');
    const fname = `vel_v4_3_N${agentCount}_MU${muStr}_R${scanRadius}_G${gStr}.json`;
    triggerDownload(JSON.stringify(payload, null, 2), fname, 'application/json');
  };
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Agent Population (N)</label>
                    <input 
                      type="number" 
                      value={agentCount} 
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Momentum (μ)</label>
                    <input 
                      type="number"
                      step="0.01"
                      min="0.50"
                      ...
                      className="w-full bg-[#0d1117] border border-slate-800 rounded-lg p-3 text-green-400 font-mono focus:border-green-500 outline-none transition-all"
                    />
                    <p className="text-[10px] text-slate-600">FW-2 sweep: 0.70 → 0.99</p>
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Scan Radius (r)</label>
                    <input 
                      type="number"
                      step="1"
                      min="1"
                      ...
                      className="w-full bg-[#0d1117] border border-slate-800 rounded-lg p-3 text-yellow-400 font-mono focus:border-yellow-500 outline-none transition-all"
                    />
                    <p className="text-[10px] text-slate-600">FW-3 sweep: 2 → 10</p>
                  </div>
                </div>
                <div className="space-y-2">
                ...
                <div className="flex items-center gap-3 p-3 bg-blue-950/30 border border-blue-900/40 rounded-lg">
                  ...
                  <p className="text-[10px] text-slate-400">
                    Active: N=<span className="text-blue-400 font-mono">{agentCount}</span> &nbsp;
                    G=<span className="text-blue-400 font-mono">{repulsionStrength}</span> &nbsp;
                    μ=<span className="text-green-400 font-mono">{momentum}</span> &nbsp;
                    r=<span className="text-yellow-400 font-mono">{scanRadius}</span>
                  </p>
                </div>
                <div className="pt-4">
                  <button 
              ...
              <div className="max-h-[200px] overflow-y-auto space-y-2 pr-2">
                {logEntries.length === 0 ? (
                  ...
                  logEntries.map(entry => (
                    <div key={entry.id} className="grid grid-cols-6 gap-1 p-2 bg-slate-900/50 rounded border border-slate-800 text-[10px] font-mono">
                      <span className="text-slate-500">N:{entry.n}</span>
                      <span className="text-slate-500">G:{entry.g}</span>
                      <span className="text-green-600">μ:{entry.mu}</span>
                      <span className="text-yellow-600">r:{entry.r}</span>
                      <span className="text-blue-400 font-bold">η:{entry.eff}</span>
          ...
          <div className="lg:col-span-7 space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400 leading-relaxed">
                      Every agent writes to a shared binary matrix. Cells flip from 0→1 on first visit.
                    </p>
                  </div>
                  <div>
                    <h4 className="text-white text-xs font-bold mb-2 flex items-center gap-2">
                    ...
                    <p className="text-[11px] text-slate-400 leading-relaxed">
                      Each agent scans a <span className="text-yellow-400 font-mono">r={scanRadius}</span>-cell radius and steers toward the nearest c=0 cell.
                    </p>
                  </div>
                </div>
                <div className="space-y-4">
                  <div>
                    <h4 className="text-white text-xs font-bold mb-2 flex items-center gap-2">
                      <div className="w-1.5 h-1.5 bg-blue-500 rounded-full"></div>
                      Momentum Conservation (μ=<span className="text-green-400">{momentum}</span>)
                    </h4>
                    <p className="text-[11px] text-slate-400 leading-relaxed">
                      Velocity decays at <span className="text-green-400 font-mono">{momentum}</span> per step. Prevents oscillation while keeping agents in smooth sweeping arcs.
                    </p>
                  </div>
                  <div>
                    <h4 className="text-white text-xs font-bold mb-2 flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-[#161b22] p-6 rounded-xl border border-slate-800 flex items-center gap-4">
                ...
                <div>
                  <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">η Benchmark</div>
                  <div className="text-sm font-bold text-white">≥ 1.0</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <MetricCard 
              ...
              highlight={parseFloat(metrics.meanEfficiency) >= 1.0}
            />
            ...
            <div className="lg:col-span-2 bg-[#161b22] p-8 rounded-xl border border-slate-800 flex flex-col h-[400px]">
              ...
              <div className="flex-1 flex items-end gap-1 px-2 border-l border-b border-slate-800 relative">
                {stats.current.history.map((val, i) => (
                  <div 
                    key={i} 
                    className={`flex-1 rounded-t-sm transition-all duration-300 ${val >= 1.0 ? 'bg-green-500' : 'bg-blue-600'}`}
                    style={{ height: `${Math.min(100, (val / 2.0) * 100)}%`, opacity: 0.3 + (i / 80) * 0.7 }}
                  />
                ))}
              </div>
              <div className="flex justify-between mt-4 text-[10px] text-slate-500 font-mono">
                <span>Trial Baseline</span>
                <span className="text-green-500/60">Green = η ≥ 1.0</span>
            ...
            <div className="bg-[#161b22] p-8 rounded-xl border border-slate-800 flex flex-col justify-between">
              <div>
                ...
                <div className="p-4 bg-slate-900 rounded-lg border border-slate-800 space-y-2 text-xs">
                  <div className="flex justify-between"><span className="text-slate-400">Algorithm</span><span className="text-blue-400">Vel-v4.3 MAP</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">N</span><span className="text-blue-400">{agentCount}</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">G</span><span className="text-blue-400">{repulsionStrength}</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">Momentum μ</span><span className="text-green-400">{momentum}</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">Scan Radius r</span><span className="text-yellow-400">{scanRadius}</span></div>
                  <div className="flex justify-between"><span className="text-slate-400">Trials</span><span className="text-blue-400">{samplesProcessed.toLocaleString()}</span></div>
